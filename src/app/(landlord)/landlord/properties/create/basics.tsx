...
import { Bath, BedDouble, Building, Building2, Home, Info, Minus, Plus, Rows3, Save } from "lucide-react-native";
...
function Option({ label, active, onPress, icon: Icon }: { label: string; active: boolean; onPress: () => void; icon?: ComponentType<{ color: string; size: number }> }) {
  return (
    <Pressable
      className="flex-row items-center gap-2 rounded-full border px-4 py-3"
      style={{ backgroundColor: active ? "#dce1ff" : colors.surface, borderColor: active ? colors.primary : colors.borderStrong }}
      onPress={onPress}
    >
...
function Counter({ label, value, onChange, icon: Icon }: { label: string; value: number; onChange: (value: number) => void; icon: ComponentType<{ color: string; size: number }> }) {
  return (
    <View className="min-w-0 flex-1">
      <AppText variant="label" muted className="mb-2 ml-1">
        {label}
      </AppText>
      <View className="h-12 flex-row items-center rounded-xl border" style={{ backgroundColor: colors.surface, borderColor: colors.borderStrong }}>
        <Pressable className="h-full w-12 items-center justify-center" onPress={() => onChange(Math.max(0, value - 1))}>
        ...
        <Pressable className="h-full w-12 items-center justify-center" onPress={() => onChange(value + 1)}>
    ...
    <AddPropertyShell
      ...
      footer={
        <View className="gap-3">
          <AppButton title="Continue to Step 2" icon={<Plus color={colors.goldDark} size={18} />} onPress={onNext} />
          <AppButton title="Save as Draft" variant="secondary" icon={<Save color={colors.primary} size={18} />} onPress={saveDraft} />
        </View>
      }
    >
        <AddPropertyPanel>
              ...
              <View>
                ...
                <View className="flex-row flex-wrap gap-2">
                  {propertyTypes.map((item) => (
                    <Option key={item.value} label={item.label} icon={item.icon} active={field.value === item.value} onPress={() => field.onChange(item.value)} />
                  ))}
                </View>
              </View>
            )}
          ...
          <View className="flex-row gap-3">
            <Controller control={form.control} name="bedrooms" render={({ field }) => <Counter label="Bedrooms" value={field.value} icon={BedDouble} onChange={field.onChange} />} />
            <Controller control={form.control} name="bathrooms" render={({ field }) => <Counter label="Bathrooms" value={field.value} icon={Bath} onChange={field.onChange} />} />
          </View>
          <Controller
            control={form.control}
          ...
          <AddPropertyNote>
            <View className="flex-row items-start gap-3">
              ...
              <AppText muted className="min-w-0 flex-1">
                Providing accurate details increases trust. Listings with clear descriptions get more enquiries in Accra.
              </AppText>
            </View>
          </AddPropertyNote>
          </View>
