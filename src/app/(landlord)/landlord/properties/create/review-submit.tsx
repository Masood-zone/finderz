...
} from "@/components/general/file-upload";
import {
  AddPropertyPanel,
  AddPropertyShell,
...
function createSubmissionId() {
  ...
  const randomPart =
    `${Math.random().toString(36).slice(2)}${Math.random().toString(36).slice(2)}`.slice(
      0,
      20,
    );
...
function AmenityRow({
  ...
  return (
    <Pressable
      ...
      style={{ backgroundColor: active ? "#dce9ff" : colors.surfaceBlue }}
      onPress={onPress}
    >
      <View
...
export default function ReviewSubmitScreen() {
  ...
  const [submissionId] = useState(
    () => draft.submissionId ?? createSubmissionId(),
  );
  ...
  const [contactPreferences, setContactPreferences] = useState(
    draft.contactPreferences ?? "PHONE",
  );
  const [inspectionAvailability, setInspectionAvailability] = useState(
    draft.inspectionAvailability ?? "",
  );
  ...
  const currentDraft = () => ({
    ...
    images: images
      ...
      .map((file, index) => ({
        ...
        position: index,
        isCover:
          (coverUri || images[0]?.uri) === file.uri ||
          (coverUri || images[0]?.upload?.secure_url) ===
  ...
  const persist = async (submitForApproval: boolean) => {
    ...
    if (submitForApproval && payload.images.length === 0) {
      Alert.alert(
        ...
        "Upload at least one property photo before submitting for approval.",
      );
      return;
    }
    ...
    if (submitForApproval && (!payload.latitude || !payload.longitude)) {
      Alert.alert(
        "Property pin required",
        "Go back to Location & Pricing and select the property's exact position on the map before submitting for approval.",
      );
      return;
    }
    ...
    if (submitForApproval && !confirmed) {
      Alert.alert(
        ...
        "Confirm that the listing information is accurate before submitting.",
      );
      return;
    }
    ...
    try {
      ...
      router.replace(
        submitForApproval
          ? ("/landlord/property-submitted" as Href)
      ...
      footer={
        <View className="flex-row gap-3">
          ...
          <AppButton
            ...
            icon={<ShieldCheck color={colors.goldDark} size={18} />}
            style={{ flex: 2 }}
      ...
      <View className="gap-7">
        <View>
          ...
          <AppText muted className="mb-4">
            Upload high-quality photos. Listings with better images get more
            enquiries in Ghana.
          </AppText>
          ...
          <View className="mt-4 flex-row flex-wrap gap-3">
            ...
            {!images.length ? (
              <View
                className="w-full items-center justify-center rounded-2xl border border-dashed p-6"
                style={{
          ...
          <View className="mt-4 gap-3">
            ...
            <View className="flex-row justify-between gap-3">
              ...
              <AppText
                ...
                style={{ fontFamily: "Manrope_700Bold" }}
              >
                {draft.area || "Area"}, {draft.city || "City"}
              </AppText>
            </View>
            <View className="flex-row justify-between gap-3">
            ...
            <View className="flex-row justify-between gap-3">
              ...
              >
                GHS {Number(draft.rentAmountCedis || 0).toLocaleString("en-GH")}{" "}
                / {draft.paymentPeriod.toLowerCase()}
              </AppText>
            </View>
          </View>
        </AddPropertyPanel>
        ...
        <View className="gap-4">
          <AppInput
            ...
            onChangeText={setContactPreferences}
          />
          <AppInput
            label="Inspection Availability"
            value={inspectionAvailability}
            onChangeText={setInspectionAvailability}
          />
          <AppInput
            label="House Rules"
            value={houseRules}
          ...
          <Checkbox
            ...
            label={
              <AppText muted className="min-w-0 flex-1">
                I confirm that the information provided is accurate and I have
                the legal right to list this property.
              </AppText>
            }
          />
        </View>
