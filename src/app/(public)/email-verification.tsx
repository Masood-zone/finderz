...
export default function EmailVerificationScreen() {
  ...
  const resend = async () => {
    if (!email) {
      setError(
        "Open this screen from a FinderZ email flow so we know which address to verify.",
      );
      return;
    }
    ...
    <SafeAreaScreen scroll>
      ...
      <View className="mt-12 items-center">
        <View
          ...
          style={{ borderRadius: radius.xxl, backgroundColor: colors.gold }}
        >
        ...
        <AppText muted className="mt-3 text-center">
          ...
          finish securing your FinderZ account.
        </AppText>
      </View>
      ...
      <View className="mt-10 gap-3">
        ...
        {message ? (
          <AppText
            ...
            style={{ color: colors.success, fontFamily: "Manrope_700Bold" }}
          >
            {message}
          </AppText>
        ) : null}
      ...
      <View
        className="mt-8 flex-row gap-3 rounded-2xl p-4"
        style={{ backgroundColor: colors.surfaceBlue }}
      >
        <ShieldCheck color={colors.primary} size={22} />
        <AppText muted className="min-w-0 flex-1">
          FinderZ uses email verification to protect Ghanaian real estate
          enquiries and landlord accounts.
        </AppText>
      </View>
