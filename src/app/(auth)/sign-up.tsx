...
export default function SignUpScreen() {
  ...
  const setHasSeenPublicOnboarding = useOnboardingStore(
    (state) => state.setHasSeenPublicOnboarding,
  );
    ...
    <KeyboardAwareScreen>
      <View className="flex-1 px-6 py-8">
        <AuthHeader
          ...
          subtitle={`Set up your ${getRoleLabel(selectedRole).toLowerCase()} on FinderZ.`}
        />

        <View
          className="mt-5 flex-row items-center gap-3 self-start rounded-full px-3 py-2"
        ...
        <View className="mt-6 gap-4">
          <Controller
            ...
            render={({ field: { onChange, onBlur, value } }) => (
              <AppInput
                ...
                left={<UserRound color={colors.outline} size={20} />}
              />
            )}
          />
          <Controller
          ...
          <Controller
            ...
            render={({ field: { onChange, value } }) => (
              <Checkbox
                ...
                label={
                  <AppText muted>
                    I agree to the FinderZ terms and privacy policy.
                  </AppText>
                }
              />
            )}
          ...
          <AppButton
            title="Create Account"
            loading={isSubmitting || assignRoleMutation.isPending}
            onPress={onSubmit}
          />
