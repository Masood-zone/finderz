...
import { useReplyTenantEnquiry, useTenantEnquiry } from "@/services/queries/hooks";
...
export default function TenantEnquiryDetailScreen() {
  ...
  const reply = useReplyTenantEnquiry();
      ...
      <KeyboardAvoidingView className="flex-1" behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <View className="flex-row items-center gap-3 border-b px-4 py-4" style={{ backgroundColor: colors.background, borderColor: colors.border }}>
          ...
          <Pressable
            ...
            style={{ borderRadius: radius.xl, backgroundColor: canCallLandlord ? colors.primary : colors.surfaceBlue, opacity: canCallLandlord ? 1 : 0.55 }}
            onPress={() => void callLandlord()}
          >
            <Phone color={canCallLandlord ? "#fff" : colors.primary} size={20} />
          </Pressable>
        </View>
        ...
        <ScrollView contentContainerStyle={{ padding: 16, gap: 12, paddingBottom: 20 }} showsVerticalScrollIndicator={false}>
          <View className="rounded-2xl border p-4" style={{ backgroundColor: colors.surface, borderColor: colors.border }}>
            ...
            <AppText variant="caption" muted>
              {enquiry.data.enquiry.property.area}, {enquiry.data.enquiry.property.city}
            </AppText>
          </View>

          {enquiry.data.messages.map((item) => {
            const mine = item.senderId !== enquiry.data.enquiry.landlord.id;
            return (
