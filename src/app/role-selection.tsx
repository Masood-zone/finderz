...
function getRoleHome(role: string) {
  if (role === "LANDLORD") {
    return "/landlord/verification-status";
  }
...
export default function RoleSelectionScreen() {
  ...
  const [selected, setSelected] = useState<PublicOnboardingRole>(
    storedRole ?? "TENANT",
  );
    ...
    <SafeAreaScreen scroll>
      ...
      <View className="mt-12">
        ...
        <AppText muted className="mt-3">
          Pick the experience that matches your housing goal. You can use
          FinderZ as a tenant or landlord.
        </AppText>
      </View>
      ...
      <View className="mt-10 gap-4">
        <RoleCard
          ...
          icon={
            <Home
              color={selected === "TENANT" ? "#fff" : colors.primary}
              size={30}
            />
          }
        ...
        <RoleCard
          ...
          icon={
            <Building2
              color={selected === "LANDLORD" ? "#fff" : colors.primary}
              size={30}
            />
          }
